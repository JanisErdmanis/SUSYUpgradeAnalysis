...
void UpgradePerformanceFunctions::setPileupTemplatesPath(TString pileupTemplateFilenamePath) {
  ...
  if ( TMath::Abs(m_avgMu - 140.) < 0.1)
    sPuPath = sPuPath+"PULibrary140.root";

  if ( TMath::Abs(m_avgMu - 200.) < 0.1)
    sPuPath = sPuPath+"PULibrary200.root";
...
float UpgradePerformanceFunctions::getTrackJetConfirmEff(float ptMeV, float eta, TString jetType) {
  ...
  //  std::cout << " jet type: " << jetType << " pt: " << ptGeV << " GeV and eta: " << eta << " surv. probability is: " << tceff << std::endl;


  // if (fabs(eta)> 2.4) {

  //   if (!m_bUseLetf) {
  //     // No Forward Calorimeter-based track confirmation
  //     tceff = 1.01;      
  //   } else { 
  //     // LETF tracking 
  //     if (jetType.Contains("HS")) {
  // 	if (ptGeV < m_fPileupTCMaxPtGeV)
  // 	  tceff = m_fHSEff; 
  // 	else
  // 	  tceff = 1.01;
  //     } else if (jetType.Contains("PU")) {
  // 	// change
  // 	if (m_bHS90){
  // 	  if (fabs(eta) <= 3.2) {
  // 	    if (ptGeV > 20. && ptGeV < 30.) {
  // 	      tceff = 0.15;
  // 	    } else if (ptGeV >= 30. && ptGeV < 40.) {
  // 	      tceff = 0.153;
  // 	    } else if (ptGeV >= 40. && ptGeV < 50.) {
  // 	      tceff = 0.157;
  // 	    } else if (ptGeV >= 50. && ptGeV < m_fPileupTCMaxPtGeV) {
  // 	      tceff = 0.053;
  // 	    } else
  // 	      tceff = 1.01; // no confirmation above m_fPileupTCMaxPtGeV
  ...
  // 	      tceff = 0.125;
  // 	    } else
  // 	      tceff = 1.01; // no cut above 50 GeV
  // 	  } else
  // 	    tceff = 1.01; // no TC for eta>4.0
  // 	}
